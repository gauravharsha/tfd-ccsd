...
from tfdccsd.ThermalCISD import betaci, numberci
...
def test_ci_beta_residuals_zeroT():
    ...
    #
    # Test to check the Configuration Interaction Residuals in the
    ...
    #       constructed here but R2 will not be zero as one needs to have
    #       up to 4th order wavefunction to get CI equivalent of CC.

    assert np.max(np.abs(r1)) <= 5e-8


def test_ci_number_residuals_zeroT():
